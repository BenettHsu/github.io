...
if (shortcutKey) {
    ...
    $(document).keyup(function (e) {
        if (!$searchInput.is(":focus") && !$tagSearchInput.is(':focus') && !$('#comments textarea').is(':focus')) {
            if (e.which === 83) { /* s - 显示/隐藏 全屏 */
                $fullBtn.trigger("click");
            } else if ((e.which === 81 || e.which === 113) && ($(".nav").css('margin-left')==='0px') && !$('.title-list').hasClass('friend')) { /* q */
                inputChange()
            } else if (e.which === 84) { /* t - 切换大纲视图 */
...
function inputChange() {
    ...
    if (val === "") {
        ...
        if (val.substr(1).length !== 0) {
            $(".nav-right nav a").css("display", "none");
            $(".nav-right nav").find("a." + 'All' + ":contains_tag('" + val.substr(1) + "')").css("display", "block");
        }
    } else if (val.substr(0, 1) === "@") {
        searchType = '作者'
        containType= '为'
